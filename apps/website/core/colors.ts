// Copyright © Aptos
// SPDX-License-Identifier: Apache-2.0

export const secondaryTextColor = {
  dark: 'gray.400',
  light: 'gray.400',
};

export const secondaryBgColor = {
  dark: 'gray.400',
  light: 'gray.400',
};

export const secondaryLinkColor = {
  dark: 'teal.200',
  light: 'teal.600',
};

export const secondaryMDXNavigationFontColor = {
  dark: 'teal.200',
  light: 'teal.400',
};

export const secondaryMDXNavigationBorderColor = {
  dark: 'whiteAlpha.400',
  light: 'blackAlpha.300',
};

export const secondaryTextColorWalletDemo = {
  dark: 'gray.400',
  light: 'gray.500',
};

export const secondaryWalletHomeCardBgColor = {
  dark: 'gray.800',
  light: 'gray.50',
};

export const secondaryAddressFontColor = {
  dark: 'gray.400',
  light: 'gray.500',
};

export const secondaryDividerColor = {
  dark: 'whiteAlpha.300',
  light: 'gray.200',
};

export const secondaryErrorMessageColor = {
  dark: 'red.200',
  light: 'red.500',
};

export const secondaryWalletNoteBgColor = {
  dark: 'blue.400',
  light: 'blue.100',
};

export const secondaryBorderColor = {
  dark: 'whiteAlpha.200',
  light: 'blackAlpha.200',
};

export const secondaryHeaderInputBgColor = {
  dark: 'gray.700',
  light: 'gray.100',
};

export const secondaryHeaderInputHoverBgColor = {
  dark: 'gray.600',
  light: 'gray.200',
};
