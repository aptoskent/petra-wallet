...
export default function TransferAvatar({
  ...
  const validRecipientAddress = isAddressValid(recipient)
  ...
  const getAvatarBadgeIcon = useCallback(() => {
    if (!validRecipientAddress) {
      return (
        <SmallCloseIcon color="white" fontSize="xs" aria-label="invalid" />
      );
    }
    if (!doesRecipientAccountExist) {
      return (
        <SmallAddIcon color="white" fontSize="xs" aria-label="not-exist" />
      );
    }
    return <CheckIcon color="white" fontSize="xs" aria-label="success" />;
      ...
      <Avatar
        icon={
          <GraceHopperBoringAvatar
            type={doesRecipientAccountExist ? 'beam' : 'marble'}
          />
        }
      >
        {avatarBadge}
